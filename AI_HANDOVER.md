...
- **Sub-Modul Kendaraan Pinjam Pakai (`/vehicles/pinjam-pakai`):** Pendataan Naskah Perjanjian Pinjam Pakai (NPPP / BAST) kendaraan dinas untuk Instansi Vertikal (Kejaksaan, Polres, KPU, Bawaslu, Kodim, dll.) maupun antar OPD sesuai Permendagri No. 19/2016 jo. Permendagri 7/2024 & Permendagri 47/2021. Dilengkapi kalkulasi otomatis sisa hari, notifikasi jatuh tempo (≤30 hari), pengunggahan PDF BAST, fitur pengembalian aset 1-klik, dan **AI Smart Semantic Import Excel** (`/vehicles/pinjam-pakai/import`).
- **Komponen Searchable Dropdown Terpadu (TomSelect):** Seluruh elemen `<select>` pada modul kendaraan eRANDIS (`/vehicles`, `/vehicles/create`, `/vehicles/edit`, `/vehicles/pinjam-pakai`, dll.) telah diredesain menggunakan **TomSelect** (`searchable-select`). Dilengkapi dukungan pencarian real-time, kompatibilitas mode terang/gelap, sinkronisasi otomatis pada modal Bootstrap (`shown.bs.modal`), penanganan event `change` native secara aman, serta helper JavaScript terpusat (`window.initSearchableSelects`, `window.setSearchableSelectValue`, `window.syncSearchableSelectOptions`).
...
- **Struktur Modular Form Request (`app/Http/Requests/Elabel/`):**
  - Seluruh validasi request eLABEL telah dipindahkan ke subdirektori modular `App\Http\Requests\Elabel` (`StoreElabelBpkbRequest`, `UpdateElabelBpkbRequest`, `StoreElabelSertifikatRequest`, `UpdateElabelSertifikatRequest`, `StoreElabelSuratPenyerahanRequest`, `UpdateElabelSuratPenyerahanRequest`).
