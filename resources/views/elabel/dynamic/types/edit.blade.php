...
@section('title', 'Edit Jenis Arsip & Form Builder - eLABEL')
...
<div class="container-fluid px-0">
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1 small">
                    ...
                    <li class="breadcrumb-item active text-navy fw-medium" aria-current="page">Edit {{ $type->nama }}</li>
                </ol>
            </nav>
            <h4 class="fw-bold text-navy mb-0 d-flex align-items-center gap-2">
                <i class="bi bi-pencil-square text-warning"></i> Edit Jenis Arsip & Form Builder
            </h4>
        </div>
    ...
    <form action="{{ route('elabel.dynamic.types.update', $type->id) }}" method="POST" id="typeForm">
        @csrf
        @method('PUT')
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <input type="text" name="kode" value="{{ old('kode', $type->kode) }}" class="form-control text-uppercase font-monospace" placeholder="Cth: IMB, KONTRAK, SPJ, SK" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $type->nama) }}" class="form-control" placeholder="Cth: Dokumen IMB & PBG Bangunan" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold text-dark">Deskripsi Singkat</label>
                            <textarea name="deskripsi" class="form-control" rows="3" placeholder="Uraian berkas atau aturan penyimpanan kategori ini...">{{ old('deskripsi', $type->deskripsi) }}</textarea>
                        </div>
                        ...
                        <div class="row g-2 mb-3">
                            <div class="col-6">
                                ...
                                <select name="icon" class="form-select">
                                    <option value="bi-folder2" {{ old('icon', $type->icon) == 'bi-folder2' ? 'selected' : '' }}>📁 Folder</option>
                                    <option value="bi-file-earmark-text" {{ old('icon', $type->icon) == 'bi-file-earmark-text' ? 'selected' : '' }}>📄 Dokumen Teks</option>
                                    <option value="bi-building" {{ old('icon', $type->icon) == 'bi-building' ? 'selected' : '' }}>🏢 IMB / Gedung</option>
                                    <option value="bi-briefcase" {{ old('icon', $type->icon) == 'bi-briefcase' ? 'selected' : '' }}>💼 Kontrak / SPK</option>
                                    <option value="bi-receipt" {{ old('icon', $type->icon) == 'bi-receipt' ? 'selected' : '' }}>🧾 Kuitansi / SPJ</option>
                                    <option value="bi-people" {{ old('icon', $type->icon) == 'bi-people' ? 'selected' : '' }}>👥 Kepegawaian</option>
                                    <option value="bi-patch-check" {{ old('icon', $type->icon) == 'bi-patch-check' ? 'selected' : '' }}>🏅 Ijazah / Sertifikat</option>
                                    <option value="bi-shield-check" {{ old('icon', $type->icon) == 'bi-shield-check' ? 'selected' : '' }}>🛡️ Dokumen Rahasia</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <label class="form-label fw-semibold text-dark">Warna Tema</label>
                                <select name="warna_badge" class="form-select">
                                    <option value="primary" {{ old('warna_badge', $type->warna_badge) == 'primary' ? 'selected' : '' }}>Biru (Primary)</option>
                                    <option value="success" {{ old('warna_badge', $type->warna_badge) == 'success' ? 'selected' : '' }}>Hijau (Success)</option>
                                    <option value="warning" {{ old('warna_badge', $type->warna_badge) == 'warning' ? 'selected' : '' }}>Kuning/Oranye</option>
                                    <option value="info" {{ old('warna_badge', $type->warna_badge) == 'info' ? 'selected' : '' }}>Biru Muda (Info)</option>
                                    <option value="danger" {{ old('warna_badge', $type->warna_badge) == 'danger' ? 'selected' : '' }}>Merah (Danger)</option>
                                    <option value="dark" {{ old('warna_badge', $type->warna_badge) == 'dark' ? 'selected' : '' }}>Hitam / Gelap</option>
                                </select>
                            </div>
                        </div>

                        <div class="form-check form-switch pt-2">
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActiveSwitch" {{ old('is_active', $type->is_active) ? 'checked' : '' }}>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-header bg-white border-bottom py-3 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <div>
                            ...
                            <span class="text-muted small">Atur kolom formulir kustom untuk jenis arsip ini.</span>
                        </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div id="emptyFieldsNotice" class="text-center py-4 border rounded-3 bg-light-subtle d-none">
    ...
    <div class="field-row card border rounded-3 p-3 bg-white shadow-xs position-relative">
        <div class="row g-2 align-items-start">
            ...
            <div class="col-md-3 options-container d-none">
                <label class="form-label text-xs fw-bold text-secondary text-uppercase mb-1">Pilihan (Pisahkan Koma)</label>
                <input type="text" name="schema_fields[INDEX][options]" class="form-control form-control-sm field-options-input" placeholder="Opsi 1, Opsi 2, Opsi 3">
            </div>
        ...
        <div class="row g-2 mt-2 pt-2 border-top">
            <div class="col-md-6">
                <input type="text" name="schema_fields[INDEX][placeholder]" class="form-control form-control-sm field-placeholder-input" placeholder="Placeholder panduan pengisian (opsional)...">
            </div>
            <div class="col-md-4">
                <input type="text" name="schema_fields[INDEX][help_text]" class="form-control form-control-sm field-help-input" placeholder="Teks bantuan kecil di bawah input...">
            </div>
            <div class="col-md-2 d-flex align-items-center">
                <div class="form-check form-check-inline">
                    <input class="form-check-input field-required-check" type="checkbox" name="schema_fields[INDEX][required]" value="1" id="req_INDEX">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const existingSchema = @json($type->schema_fields ?? []);
    ...
    function addField(data = null) {
        ...
        const helpInput = row.querySelector('.field-help-input');
    ...
    if (existingSchema && existingSchema.length > 0) {
        existingSchema.forEach(field => addField(field));
    } else {
        checkEmpty();
    }
});
